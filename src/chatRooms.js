...
const ckeckUnread = (viewTime, messages) => {
  ...
  if (!message) {
    ...
    return (
      new Date(message.timestamp).getTime() - new Date(viewTime).getTime() > 0
    );
  }
};
...
const getOptionNode = (members, friends) => {
  return friends.map((friend) => {
    if (members.some((member) => member === friend)) {
      return (
        <Option value={friend} key={friend} disabled>
          {friend}
        </Option>
      );
    } else {
...
const ChatRooms = ({
  ...
  const [updateMessagesViewTime] = useMutation(
    UPDATE_MESSAGES_VIEW_TIME_MUTATION
  );
      ...
      <>
        <RoomHeader>
          <div style={{ display: "flex", alignItems: "center" }}>
            ...
            <div>
              ...
              <Typography
                ...
                noWrap
              >{`Room members: ${members.join(", ")}`}</Typography>
            </div>
          </div>
          <div style={{ display: "flex" }}>
            <Button
              ...
              onClick={() => setOpenAddRoomMemberModal(true)}
              ghost
            >
              Add Member
            </Button>
            <Button type="primary" onClick={handleLeaveRoom} danger ghost>
              Leave Room
            </Button>
          </div>
        </RoomHeader>
        <RoomContent>
          {messages.map(({ sender, messageId, text, timestamp }) => {
            if (sender === "0308F") {
              return (
                <div className="system-message" key={messageId}>
                  {text}
                </div>
              );
            }
            ...
            return (
              <div
                className={sender === me ? "text-box-right" : "text-box-left"}
                key={messageId}
              >
                <div className="avatar">
                  <Avatar
                    sx={{ margin: "auto", cursor: "pointer" }}
                    src={ME.me.users.headShotURLs[sender]}
                    onClick={
                      sender === me
                        ? () => navigate("/me")
                        : () => navigate(`/visits/${sender}`)
                    }
                  />
                ...
                <div className="text-box">
                  {sender === me ? (
                    <Popconfirm
                      ...
                      okText="Yes"
                      cancelText="No"
                      onConfirm={() => handleDeleteMessage(roomId, messageId)}
                      onCancel={() => setOpenMask(false)}
                    >
                      <div className="text" onClick={() => setOpenMask(true)}>
                        {text}
                      </div>
                    </Popconfirm>
                  ) : (
        ...
        <AddRoomMemberModal
          style={{ display: openAddRoomMemberModal ? "block" : "none" }}
        >
          ...
          <div className="add-room-member-box">
            ...
            <Button type="primary" onClick={handleAddRoomMember}>
              Add
            </Button>
          </div>
        </AddRoomMemberModal>
      ...
      {ME && ROOM ? (
        <>
              ...
              >
                {ROOM.room.map(({ roomName, roomId, messages, viewTime }) => (
                  <ListItem
                    ...
                    divider
                    key={roomId}
                    onClick={() => handleChangeActiveRoom(roomId)}
                  >
                    <Avatar {...stringAvatar(roomName)} />
                    <ListItemText
                      primary={
                        <Typography sx={{ fontWeight: "bolder" }} noWrap>
                          {roomName}
                        </Typography>
                      }
                      secondary={
                        <Typography
                          ...
                          noWrap
                        >
                          {getSubTitle(messages)}
                        </Typography>
                      }
                      disableTypography
                    />
                    {ckeckUnread(viewTime, messages) &&
                    roomId !== activeRoom ? (
                      <div>
                        <UnreadDot />
                      </div>
          ...
          <Content style={{ background: "white" }}>
            {ROOM.room.some(({ roomId }) => roomId === activeRoom) ? (
              <div style={{ width: "100%", height: "100%", overflow: "auto" }}>
                {getRoomNode(
                  ROOM.room.find(({ roomId }) => roomId === activeRoom)
                )}
              </div>
            ) : (
              <NoRoomWrapper>
                <Typography
                  sx={{ color: "#65676b", fontWeight: "bolder", fontSize: 24 }}
                  noWrap
                >
                  選擇聊天室或開始新對話
                </Typography>
          ...
          <Mask
            style={{
              display:
                openCreateRoomModal || openAddRoomMemberModal || openMask
                  ? "block"
                  : "none",
            }}
            onClick={() => {
              if (openMask) setOpenMask(false);
            }}
          />
          <CreateRoomModal
            style={{ display: openCreateRoomModal ? "block" : "none" }}
          >
            <div className="add-room-title">Create Room</div>
            <button
              className="add-room-close-btn"
              onClick={() => setOpenCreateRoomModal(false)}
            >
              <CloseOutlined />
            </button>
