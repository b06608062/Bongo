...
const Content = ({
  ...
  <MyContent>
    <div>
      <Typography sx={{ fontSize: "large", fontWeight: "bolder" }}>
        {post.title}
      </Typography>
    </div>
    ...
    {post.image === "" ? (
      ...
      <Image
        ...
        onError={(e) => {
          e.target.classList.add("fall-image");
        }}
      />
    )}
    <div style={{ display: "flex", alignItems: "center" }}>
      <IconButton
        ...
        className="like"
        onClick={
          post.likes.some((like) => like === me)
            ? () => handleUnlike(post.postId)
            : () => handleLike(post.postId)
        }
      >
        {post.likes.some((like) => like === me) ? (
          <FavoriteIcon sx={{ color: "#ef5454" }} />
        ) : (
          <FavoriteBorderIcon />
        )}
      </IconButton>
      ...
      >
          ...
          <div className="post-comment" key={comment.commentId}>
            <div className="post-comment-avatar">
              <Avatar
                ...
                src={headShotURLs[comment.author]}
                onClick={
                  comment.author === me
                    ? () => navigate("/me")
                    : () => navigate(`/visits/${comment.author}`)
                }
              />
        ...
        <div style={{ display: "flex" }}>
          <Button
            type="primary"
            onClick={() => {
              handleCreateComment(post.postId, text);
              setText("");
            }}
...
const Title = ({ post: { author }, headShotURLs, navigate, me }) => (
  <div style={{ display: "flex", alignItems: "center", cursor: "default" }}>
    <Avatar
      sx={{ width: 36, height: 36, mr: "6px", cursor: "pointer" }}
      src={headShotURLs[author]}
      onClick={
        author === me
          ? () => navigate("/me")
          : () => navigate(`/visits/${author}`)
      }
    />
    <Typography variant="inherit" sx={{ fontSize: 18, maxWidth: 150 }} noWrap>
      {author}
    </Typography>
  </div>
);
...
const SubTitle = ({ post: { createTime, published } }) => (
  <div style={{ display: "flex", alignItems: "center", cursor: "default" }}>
    <span
      ...
      style={{ fontSize: 14 }}
    >
      {formatDate(new Date(createTime))}
    </span>
    {published === "Public" ? (
      <Tooltip title="Public" placement="top">
        <PublicIcon sx={{ width: 14, height: 14 }} />
      </Tooltip>
    ) : published === "Friends" ? (
      <Tooltip title="Friends" placement="top">
        <PeopleAltIcon sx={{ width: 14, height: 14 }} />
      </Tooltip>
    ) : (
      <Tooltip title="Private" placement="top">
        <Lock sx={{ width: 14, height: 14 }} />
      </Tooltip>
    )}
  </div>
);
    ...
    <>
      <PageHeader
        ...
        tags={post.tags.map((tag, idx) => (
          <Tag color="blue" style={{ cursor: "pointer" }} key={idx}>
            {tag}
          </Tag>
        ))}
        extra={
