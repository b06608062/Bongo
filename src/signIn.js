...
const SignIn = ({ me, setMe, setOnline }) => {
    ...
    return(
        <Wrapper> 
            <Title style={{ marginBottom: 15, color: 'white' }} className="logo">Bongo</Title>
            <Input 
                value={me}
            ...
            <Input.Password 
                ...
                className='RWD'
            />
            <SignInButton type="primary" onClick={handleSignIn} className='RWD'>Sign in</SignInButton>
            <Button type="link" className='forget'>忘記密碼</Button>
            <hr style={{ border: '1px dashed #BEBEBE', width: 300 }} className='dash'></hr>
            <SignUpButton onClick={() => setOpenModal(true)} className="sign-up">Sign up</SignUpButton>
            <Mask style={{ display: openModal?'block':'none' }}/>
            <SignUpModal style={{ display: openModal?'block':'none' }}>
                <h1 className='sign-up-title'>快速註冊</h1>
                <CloseCircleFilled style={{ position: 'absolute', top: '5px', right: '5px', fontSize: '16px', cursor: 'pointer' }} onClick={() => setOpenModal(false)}/>
                <Form
