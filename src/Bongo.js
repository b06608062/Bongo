...
import { Button, Drawer, Layout, message, Spin } from 'antd';
import { MessageOutlined } from '@ant-design/icons';
import { AppBar, Toolbar, IconButton, Badge, Menu, MenuItem, Avatar, List, ListItem, ListItemText, Typography, Tooltip } from '@mui/material';
...
const Bongo = ({ me, setOnline }) => {
    ...
    const { data: POST, subscribeToMore: SUBPOST, refetch: REFETCHPOST, loading } = useQuery(GET_POST_QUERY, { variables: { query: me } });
    ...
    const [updateNotificationsViewTime] = useMutation(UPDATE_NOTIFICATIONS_VIEW_TIME_MUTATION, { variables: { query: me } });
                            ...
                            <AppBar sx={{ bgcolor: '#081165', zIndex: 990 }}>
                                <Toolbar sx={{ overflow: 'auto' }}>
                                    ...
                                    <Tooltip title="Notifications" placement="bottom">
                                        ...
                                        >
                                            <Badge badgeContent={openNotificationDrawer?0:countUnreadNotifications(ME.me.notificationsViewTime, ME.me.notifications)} color="error">
                                                <NotificationsIcon />
                                            </Badge>
                                        </IconButton>
                                    </Tooltip>
                                ...
                                >   
                                    {<h2 style={{ fontSize: '1rem', paddingLeft: '16px', fontWeight: 'bolder' }}>所有使用者</h2>}
                                    {ME.me.users.usersList.map(userName => {
                                        if(userName!==me) return (<MenuItem key={userName} onClick={() => {setAnchorSearch(null); navigate(`visits/${userName}`);}}>
                                ...
                                >
                                    {<h2 style={{ fontSize: '1rem', paddingLeft: '16px', fontWeight: 'bolder' }}>收到的交友邀請</h2>}
                                    {ME.me.invitations.map((invitation) => (
                                        <MenuItem key={invitation} sx={{ displayRaw: 'flex', justifyContent: 'space-around', cursor: 'default' }}>
                                            ...
                                            <Button onClick={() => {acceptRequest({ variables: { from: me, to: invitation } });setAnchorAddFriend(false);}}>接受</Button>
                                            <Button onClick={() => {rejectRequest({ variables: { from: me, to: invitation } });setAnchorAddFriend(false);}}>拒絕</Button>
                                        </MenuItem>
                                    ))}
                                </Menu>
                                ...
                                <Menu
                                    ...
                                    >
                                        {<h2 style={{ fontSize: '1rem', paddingLeft: '16px', fontWeight: 'bolder' }}>我的朋友</h2>}
                                ...
                                <NotificationDrawer title="Notifications" placement="right" onClose={handleOpenNotificationDrawer} visible={openNotificationDrawer} mask={false} style={{ width: openNotificationDrawer?300:0 }}>
                                    <List sx={{ bgcolor: 'background.paper', width: '100%' }} component="nav" aria-label="mailbox folders">
                                        {ME.me.notifications.map(({ info, timestamp }) => (
                                            <ListItem button divider key={timestamp}>
                                                <ListItemText primary={info} secondary={moment(timestamp).fromNow()}/>
                                            </ListItem>
                                        ))}
                                    </List>
                                </NotificationDrawer>
