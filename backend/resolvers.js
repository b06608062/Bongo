...
const resolvers = {
  Query: {
    ...
    async post(parent, { query }, { db, pubSub }, info) {
      ...
      return posts.filter(
        (post) =>
          post.published === "Public" ||
          post.author === query ||
          (post.published === "Friends" &&
            friends.some((friend) => friend === post.author))
      );
    },
    async room(parent, { query }, { db, pubSub }, info) {
      const rooms = await db.ChatRoomModel.find({ "members.name": query }).sort(
        { "members.viewTime": -1 }
      );
      return rooms.map(
        ({ _id, roomName, roomId, members, messages, createTime }) => ({
          _id,
          roomName,
          roomId,
  ...
  Mutation: {
    async createUser(
      ...
      await new db.UserModel({
        ...
        notifications: [
          { info: `${userName}, Welcome to Bongo Cha Cha!`, timestamp: now },
        ],
    ...
    async updateHeadShot(
      parent,
      { data: { userName, headShotURL, updateType } },
      { db, pubSub },
      info
    ) {
      ...
      pubSub.publish("Public User", {
        publicUser: {
          ...
          data: {
            userName,
            headShotURL,
            headShotUpdateTime: user.headShotUpdateTime,
          },
        },
      });
      return true;
    ...
    async addFriend(parent, { data }, { db, pubSub }, info) {
      ...
      if (to.invitations.some((invitation) => invitation === data.from))
        return false;
      from.requests.push(data.to);
      from.notifications.unshift({
        info: `You sent a friend request to ${data.to}.`,
        timestamp: now,
      });
      to.invitations.push(data.from);
      to.notifications.unshift({
        info: `${data.from} want to be your friend.`,
        timestamp: now,
      });
      await from.save();
      await to.save();
    ...
    async acceptRequest(parent, { data }, { db, pubSub }, info) {
      ...
      from.notifications.unshift({
        info: `Congratulations! You and ${data.to} became friend on Bongo.`,
        timestamp: now,
      });
      from.friends.push(data.to);
      to.requests = to.requests.filter((user) => user !== data.from);
      to.notifications.unshift({
        info: `Congratulations! ${data.from} accepted your friend request.`,
        timestamp: now,
      });
    ...
    async rejectRequest(parent, { data }, { db, pubSub }, info) {
      ...
      from.invitations = from.invitations.filter((user) => user !== data.to);
      from.notifications.unshift({
        info: `You rejected ${data.to}'s friend request.`,
        timestamp: now,
      });
      to.requests = to.requests.filter((user) => user !== data.from);
      to.notifications.unshift({
        info: `Oops! ${data.from} rejected your friend request.`,
        timestamp: now,
      });
      await from.save();
      await to.save();
    ...
    async createComment(parent, { data }, { db, pubSub }, info) {
      ...
      if (post.author !== author) {
        ...
        user.notifications.unshift({
          info: `${data.author} left a comment on your post.`,
          timestamp: now,
        });
        await user.save();
      ...
      pubSub.publish("Public Post", {
        publicPost: {
          ...
          data: {
            postId,
            comment: { author, commentId, text, timestamp: now },
          },
        },
      });
      return true;
    ...
    async createRoom(
        ...
        messages: [
          {
            sender: "0308F",
            messageId: uuidv4(),
            text: `${members[0]} created the room.`,
            timestamp: now,
          },
        ],
        createTime: now,
      ...
      users.forEach(async (user) => {
        user.userName === members[0]
          ? user.notifications.unshift({
              info: `ChatRoom \"${roomName}\" created successfully!`,
              timestamp: now,
            })
          : user.notifications.unshift({
              info: `${members[0]} invited you to Cha Cha in ChatRoom \"${roomName}\".`,
              timestamp: now,
            });
        await user.save();
        pubSub.publish(user.userName, {
    ...
    async addRoomMember(
      ...
      room.messages.push({
        sender: "0308F",
        messageId: uuidv4(),
        text: `${from} added ${to} to the room.`,
        timestamp: now,
      });
      await room.save();
      const user = await db.UserModel.findOne({ userName: to });
      user.notifications.unshift({
        info: `${from} invited you to Cha Cha in ChatRoom \"${room.roomName}\".`,
        timestamp: now,
      });
      await user.save();
      pubSub.publish(to, {
          ...
          pubSub.publish(`${member.name}'s Room`, {
            room: {
              ...
              data: {
                ...
                from,
                to,
                sysMsg: room.messages[room.messages.length - 1],
              },
            },
          });
        } else {
